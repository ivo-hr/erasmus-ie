...
# Function to query and print a list of all document ids in the "Incident" collection
def query_all_documents(database):
    count = 0
    all_documents = database.Incident.find()
    for document in all_documents:
        print(document["Record_ID"])
        count += 1
...
def query_sorted_documents(database, sort_field, sort_order):
    count = 0
    sorted_documents = database.Incident.find().sort(sort_field, sort_order)
...
def query_aggregation_documents(database, match_criteria):
    count = 0
    aggregation_pipeline = [{"$match": match_criteria}]
    aggregation_result = database.Incident.aggregate(aggregation_pipeline)
    for document in aggregation_result:
        print(document["Record_ID"], "matches the criteria of", match_criteria)
        count += 1
    print(f"Number of records: {count}")
...
def query_specific_document(database, record_id):
    specific_document = database.Incident.find({"Record_ID": record_id})
...
def add_document(database, new_document):
    ...
    result = database.Incident.insert_one(new_document)
    print("Inserted Record ID:", result.inserted_id)
    
#Function to update a document in the database
def update_document(database, record_id, updated_document):
    # Update the document in the database
    result = database.Incident.update_one({"Record_ID": record_id}, {"$set": updated_document})
...
def delete_document(database, record_id):
    ...
    result = database.Incident.delete_one({"Record_ID": record_id})
    print("Deleted", result.deleted_count, "record(s)")
...
if __name__ == "__main__":
    ...
    # Add --query argument
    group.add_argument("--query", choices=["all", "specific", "species", "projection", "sorted", "aggregation"],
